...
def update_wp_post_price(wp_post_id, old_price, new_price, deal_label):
    ...
    try:
        ...
        # Questa regex cerca qualsiasi tag (p o div) che abbia il colore rosso (#b12704)
        # Sostituisce l'intero contenuto per riparare eventuali "c1.73"
        price_pattern = r'(<(p|div)[^>]*color:\s?#b12704[^>]*>)(.*?)(</\2>)'
        
        if re.search(price_pattern, content, re.IGNORECASE):
            print(f"      🔧 Fix Prezzo (Metodo Strutturale)")
            content = re.sub(price_pattern, f'\\g<1>€ {new_str}\\g<4>', content, flags=re.IGNORECASE)
        else:
            # Metodo di Ricostruzione Totale se il box rosso è sparito
            ...
            if re.search(reconstruct_pattern, content, re.IGNORECASE):
                print(f"      🔧 Ricostruzione Totale Prezzo")
        ...
        # Cerca testi comuni (Analisi in corso, Monitoraggio, ecc) dentro small o div
        ...
        # --- 4. FIX SCHEMA JSON (Gestione corruzione "offers") ---
        # Ripara sia il formato standard che quello corrotto "c2.64"
...
def run_price_monitor():
    print(f"🚀 [{datetime.now().strftime('%H:%M:%S')}] MONITORAGGIO ULTIMATE v9.1 AVVIATO...")
    while True:
        conn = None
        try:
            ...
            for p_id, asin, old_price, wp_id in products:
                ...
                if new_price and abs(float(old_price) - new_price) > 0.01:
                    ...
                    conn.commit()
                    update_wp_post_price(wp_id, old_price, new_price, deal)
                else:
                    print(f"   ⚖️ {asin}: Stabile (€{old_price}) {'['+deal+']' if deal else ''}")
