...
def process_emails():
    ...
    print(f"Connessione a {IMAP_SERVER} cartella '{IMAP_FOLDER}'...")
    ...
    try:
        ...
        if status != "OK":
            print(f"❌ Cartella {IMAP_FOLDER} non trovata.")
            return
        ...
        for i in mail_ids:
            ...
            # 2. Estrazione Nome Prodotto
            p_name = extract_product_name(subject)
            print(f"   🏷️ Prodotto rilevato: {p_name}")

            # 3. Upload Immagini Allegati su WP per ottenere URL
            wp_image_urls = []
            ...
            # 5. Costruzione HTML e Pubblicazione Bozza
